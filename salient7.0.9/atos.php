...
//$sql = "SELECT value FROM wp_".$siteid."_rg_lead_detail WHERE form_id=".$form_id." AND lead_id=".$entry_id." AND field_number=".$field_id;
$sql = "SELECT meta_value FROM wp_".$siteid."_gf_entry_meta WHERE form_id=".$form_id." AND entry_id=".$entry_id." AND meta_key LIKE '".$field_id."'";
...
//echo("Inscript : ".$inscr." | "."frais : ".$frais." | "."total : ".$total);die;
...
echo '<div align="center"><p style="line-height:20px">Please choose your payment method below. The total amount is '.$total.'€</p><p>(registration : '.$inscr.'€ + 1,75% bank charge)</p></div>';
	...
	//$parm="merchant_id=014213245611111";
		// code de production :
	$parm="merchant_id=048008951500014";
	$parm="$parm merchant_country=fr";
	$parm="$parm amount=".number_format($total,2,"","");//."00";
	$parm="$parm currency_code=978";

	$parm="$parm pathfile=../sogenactif/param/pathfilewp";
	...
	//
	$parm="$parm normal_return_url=http://atoutcom.preprod.eybios.com/".$site."/confirmation-cb";
	$parm="$parm cancel_return_url=http://atoutcom.preprod.eybios.com/".$site;
	$parm="$parm automatic_response_url=http://atoutcom.preprod.eybios.com/auto.php";
if($lang == 'fr')
	...
	//
	$path_bin = "../sogenactif/bin/request";
...
//include $_SERVER['DOCUMENT_ROOT']."/wp-content/themes/".$theme."/footer.php";
	?>
